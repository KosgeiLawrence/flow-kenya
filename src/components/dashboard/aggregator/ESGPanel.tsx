...
const COLORS_CHART = ["hsl(152,45%,22%)", "hsl(40,55%,55%)", "hsl(195,60%,50%)", "hsl(25,30%,35%)", "hsl(340,50%,50%)"];
...
const AggregatorESGPanel = () => {
  ...
  // Aggregators use collections data
  const { data: collections } = useQuery({
    queryKey: ["aggregator_esg_collections", user?.id],
    queryFn: async () => {
  ...
  const { data: cleanups } = useQuery({
    queryKey: ["aggregator_esg_cleanups", user?.id],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  // Material breakdown from collections
  ...
  const getOrgDetails = async () => {
    const name = orgInfo?.orgName || profile?.full_name || "Aggregator";
    let logo: string | null = null;
    if (orgInfo?.orgLogoUrl) logo = await loadImageAsBase64(orgInfo.orgLogoUrl);
    const contact = [
  ...
  const downloadESGReport = async () => {
    ...
    doc.setFontSize(11);
    doc.setTextColor(...PDF_COLORS.forestDeep);
    doc.text("MATERIAL COLLECTION & AGGREGATION", 15, y);
    y += 8;
    doc.setFontSize(9);
    const txRows = [
      ["Materials Collected", `${collectionKg.toFixed(1)} kg`],
      ["Collection Entries", `${filteredCollections.length}`],
      ["Material Types", `${materialMap.size}`],
      ["Waste Pickers Supported", `${new Set(filteredCollections.map(c => c.user_id)).size}`],
    ];
        ...
        <CardContent className="p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Based on collection volumes, cleanups, and environmental impact</p>
            </div>
          </div>
...
export default AggregatorESGPanel;
