...
const InvoicesPanel = () => {
  ...
  const generateInvoice = () => {
    ...
    doc.setFontSize(20); doc.text("Duara Flow", 20, 22);
    doc.setFontSize(10); doc.setTextColor(100); doc.text("Sales Invoice", 20, 30); doc.setTextColor(0);
    ...
    let y = 80;
    doc.setFillColor(34, 87, 62); doc.rect(20, y - 5, 170, 8, "F");
    doc.setTextColor(255); doc.setFontSize(9);
    doc.text("Material", 22, y); doc.text("Quantity", 85, y); doc.text("Unit Price", 120, y); doc.text("Total (KES)", 155, y);
    doc.setTextColor(0);

    let grandTotal = 0; y += 10;
    materialMap.forEach((m) => {
      const total = m.qty * m.price; grandTotal += total;
      doc.text(m.name, 22, y); doc.text(`${m.qty.toFixed(1)} ${m.unit}`, 85, y);
      doc.text(m.price.toFixed(2), 120, y); doc.text(total.toLocaleString(), 155, y); y += 8;
    });

    y += 4; doc.line(20, y - 3, 190, y - 3);
    doc.setFontSize(12); doc.text(`Grand Total: KES ${grandTotal.toLocaleString()}`, 110, y + 5);
    doc.setFontSize(7); doc.setTextColor(130); doc.text("This invoice is system-generated by Duara Flow.", 20, 280);
  ...
  const generateDeliveryNote = () => {
    ...
    doc.setFontSize(20); doc.text("Duara Flow", 20, 22);
    doc.setFontSize(10); doc.setTextColor(100); doc.text("Delivery Note", 20, 30); doc.setTextColor(0);
    doc.text(`Note #: ${noteNo}`, 20, 44);
    doc.text(`Date: ${format(new Date(), "MMM d, yyyy")}`, 20, 51);
    doc.text(`Delivered by: ${profile?.full_name || "Aggregator"}`, 20, 58);

    let y = 74;
    doc.setFillColor(34, 87, 62); doc.rect(20, y - 5, 170, 8, "F");
    doc.setTextColor(255); doc.setFontSize(9);
    doc.text("Batch ID", 22, y); doc.text("Material", 70, y); doc.text("Quantity", 120, y); doc.text("Date", 155, y);
    doc.setTextColor(0);
    ...
    collections.slice(0, 30).forEach((c) => {
      ...
      const mt = (c as any).material_types;
      doc.setFontSize(8);
      doc.text(c.batch_id, 22, y); doc.text(mt?.name || "—", 70, y);
      doc.text(`${Number(c.quantity).toFixed(1)} ${mt?.unit || "kg"}`, 120, y);
      doc.text(format(new Date(c.collected_at), "MMM d"), 155, y); y += 7;
    });

    y += 8;
    doc.setFontSize(9);
    doc.text("Received by: ____________________________", 20, y); y += 12;
    doc.text("Signature: ____________________________", 20, y); y += 12;
    doc.text("Date: ____________________________", 20, y);
    doc.setFontSize(7); doc.setTextColor(130); doc.text("System-generated delivery note — Duara Flow", 20, 285);
    doc.save(`delivery-note-${noteNo}.pdf`);
  };

  const generateMaterialInvoice = (materialTypeId: string) => {
    ...
    doc.setFontSize(20); doc.text("Duara Flow", 20, 22);
    doc.setFontSize(10); doc.setTextColor(100); doc.text(`Material Invoice — ${mt?.name}`, 20, 30); doc.setTextColor(0);
    doc.text(`Invoice #: ${invoiceNo}`, 20, 44);
    doc.text(`Date: ${format(new Date(), "MMM d, yyyy")}`, 20, 51);
    doc.text(`From: ${profile?.full_name || "Aggregator"}`, 20, 58);
    ...
    doc.setFillColor(34, 87, 62); doc.rect(20, y - 5, 170, 8, "F");
    doc.setTextColor(255); doc.setFontSize(9);
    doc.text("Batch ID", 22, y); doc.text("Quantity", 85, y); doc.text("Location", 120, y); doc.text("Date", 160, y);
    ...
    items.slice(0, 30).forEach(c => {
      ...
      doc.text(format(new Date(c.collected_at), "MMM d"), 160, y);
      y += 7;
    });

    y += 6; doc.line(20, y - 3, 190, y - 3);
    doc.setFontSize(10);
    doc.text(`Total: ${totalQty.toFixed(1)} ${mt?.unit || "kg"}`, 22, y + 4);
    doc.text(`Value: KES ${totalVal.toLocaleString()}`, 120, y + 4);
    doc.setFontSize(7); doc.setTextColor(130); doc.text("System-generated material invoice — Duara Flow", 20, 280);
    doc.save(`invoice-${mt?.name?.replace(/\s/g, "-")}-${invoiceNo}.pdf`);
  };

  // Get unique material types from collections
