...
const SuppliersPanel = () => {
  ...
  // Fetch platform aggregators and recyclers for selection
  const { data: platformUsers } = useQuery({
    queryKey: ["platform_suppliers_list"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, user_id, full_name, phone_number, email, county, organization_id, organizations(name)")
        .order("full_name");
      if (error) throw error;
      return data;
    },
          ...
          <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto p-4">
            ...
            {!editId && (
              <Tabs value={addMode} onValueChange={(v) => setAddMode(v as "manual" | "platform")} className="mb-2">
                <TabsList className="grid w-full grid-cols-2 h-8">
                  ...
                  <TabsTrigger value="platform" className="text-xs">From Platform</TabsTrigger>
                </TabsList>
              </Tabs>
            )}
            ...
            {addMode === "platform" && !editId && (
              <div className="space-y-2 mb-3">
                <Input placeholder="Search aggregators & recyclers..." value={platformSearch} onChange={(e) => setPlatformSearch(e.target.value)} className="h-8 text-sm" />
            ...
            <div className="space-y-2">
              <div><Label className="text-xs">Supplier / Company Name *</Label><Input className="h-8 text-sm" value={form.supplier_name} onChange={(e) => setForm({ ...form, supplier_name: e.target.value })} placeholder="e.g. Kibera Aggregators" /></div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div><Label className="text-xs">Payment Terms</Label><Input className="h-8 text-sm" value={form.payment_terms} onChange={(e) => setForm({ ...form, payment_terms: e.target.value })} placeholder="e.g. Net 30" /></div>
              </div>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {s.platform_user_id && <Badge variant="default" className="text-[10px]">Platform</Badge>}
                      </div>
