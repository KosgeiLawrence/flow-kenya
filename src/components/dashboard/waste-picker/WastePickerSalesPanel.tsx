...
const WastePickerSalesPanel = () => {
  ...
  const [customerSource, setCustomerSource] = useState<"crm" | "aggregators">("crm");
  const [pendingSales, setPendingSales] = useState<(SaleState & { vat: VatConfig })[]>(() => {
    try {
      const saved = localStorage.getItem(`pending_wp_sales_${user?.id || "anon"}`);
  ...
  useEffect(() => {
    if (!user) return;
    localStorage.setItem(`pending_wp_sales_${user.id}`, JSON.stringify(pendingSales));
  }, [pendingSales, user]);
  ...
  // Fetch platform aggregators for selling to
  const { data: platformAggregators } = useQuery({
    queryKey: ["platform_aggregators_for_wp_sale"],
    queryFn: async () => {
      ...
      const { data: roles } = await supabase
        ...
        .select("user_id")
        .eq("role", "aggregator");
      const aggIds = new Set(roles?.map(r => r.user_id) || []);
      return (data || []).filter(p => aggIds.has(p.user_id) && p.user_id !== user?.id);
    },
    enabled: !!user,
  });

  const { data: collections } = useQuery({
    queryKey: ["wp_inventory_for_sale", user?.id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("collections")
  ...
  const generatePdf = async (docType: "Quotation" | "Invoice" | "Receipt") => {
    ...
    const docNo = `${prefix}-${sale.refNo.replace("WS-", "")}`;
    ...
    y = addDocMeta(doc, [
      ...
      { label: docType === "Quotation" ? "From" : "Seller", value: orgInfo?.orgName || profile?.full_name || "Waste Picker" },
  ...
  const handleGenerateReceipt = async () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["wp_inventory_for_sale"] });
    ...
    toast.success("Sale completed! Income recorded in My Earnings.");
  };
  ...
  const exportSalesCsv = () => {
    ...
    a.href = url; a.download = `wp-sales-${format(new Date(), "yyyy-MM-dd")}.csv`; a.click();
  ...
  const exportSalesPdf = async () => {
    ...
    finalizeCleanPdf(doc);
    doc.save(`wp-sales-report-${format(new Date(), "yyyy-MM-dd")}.pdf`);
      ...
      <div className={`grid gap-6 ${pendingSales.length > 0 ? "grid-cols-1 lg:grid-cols-[1fr_340px]" : "grid-cols-1"}`}>
        <Card className="shadow-soft">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-lg">Sell from Collections</CardTitle>
          </CardHeader>
          <CardContent>
            {!materialStock.size ? (
              <p className="text-sm text-muted-foreground">No materials in your collections yet. Log collections first.</p>
            ) : (
        ...
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          ...
          {salesHistory && salesHistory.length > 0 && (
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={exportSalesCsv}><Download className="w-3 h-3 mr-1" /> CSV</Button>
                ...
                <div className="space-y-3">
                  {!showCrmPicker && (
                    <Button variant="outline" size="sm" className="w-full gap-2" onClick={() => { setShowCrmPicker(true); setCustomerSource("crm"); }}>
                      <Users className="w-4 h-4" /> Select Existing Customer or Aggregator
                    </Button>
                  )}
                  {showCrmPicker && (
                      ...
                      <CardContent className="p-3 space-y-2">
                        <div className="flex items-center justify-between">
                          <div className="flex gap-1">
                            ...
                            <Button variant={customerSource === "aggregators" ? "default" : "outline"} size="sm" className="h-6 text-xs px-2" onClick={() => setCustomerSource("aggregators")}>
                        ...
                        <div className="relative">
                          ...
                          <Input placeholder={customerSource === "crm" ? "Search customers..." : "Search aggregators..."} value={crmSearch} onChange={e => setCrmSearch(e.target.value)} className="pl-7 h-8 text-sm" />
                        </div>
                        <div className="max-h-40 overflow-y-auto space-y-1">
                          {customerSource === "crm" ? (
                            ...
                            platformAggregators && platformAggregators.length > 0 ? platformAggregators
                              ...
                              .map(r => (
                                <button key={r.user_id} className="w-full text-left p-2 rounded-md hover:bg-primary/10 transition-colors text-sm" onClick={() => {
                                  ...
                                  <Badge variant="outline" className="text-[9px] mt-0.5">Aggregator</Badge>
                                </button>
                              )) : <p className="text-xs text-muted-foreground text-center py-2">No aggregators found</p>
                          )}
                        </div>
                      </CardContent>
                    </Card>
                  ...
                  <Card className="bg-primary/10 border-primary/30">
                    <CardContent className="p-6 text-center space-y-3">
                      ...
                      <div className="text-xs text-muted-foreground space-y-1">
                        <p>✅ Income recorded in My Earnings</p>
...
export default WastePickerSalesPanel;
