...
/**
 * Premium branded header — Twende Green Ecocycle logo + contact.
...
export const addBrandedHeader = async (
  ...
  if (logo) {
    ...
    doc.text("TWENDE GREEN ECOCYCLE", S.margin, y + 4);
  }
...
export const addBrandedFooter = async (doc: jsPDF) => {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(`${CONTACT.location}  ·  Generated by Twende Green Ecocycle`, cx, footerY + 10, { align: "center" });
